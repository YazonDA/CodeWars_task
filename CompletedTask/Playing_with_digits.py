'''CodeWars task - Playing with digits
...
if it exists, such as the sum of the digits of n taken to the
...
def dig_pow(n, p):
	x, y = divmod(
		sum(
			int(i) ** (j+p) for j,i in enumerate(list(str(n)))),
			n)
	return x if not y else -1
